...
local bandwidth_tests = {
  {
    name="defines a minimum bitrate from FFmpeg", filter=hls.bandwidth, manifest=ffmpeg_master, context={min=1500000, max=math.huge},
    check=function(modified_manifest)
      local rendition_count = 0
      for _ in string.gmatch(modified_manifest, "(BANDWIDTH=%d+)") do
  ...
  {
    name="defines a maximum bitrate from FFmpeg", filter=hls.bandwidth, manifest=ffmpeg_master, context={min=0, max=1500000},
    check=function(modified_manifest)
      local rendition_count = 0
      for _ in string.gmatch(modified_manifest, "(BANDWIDTH=%d+)") do
        rendition_count = rendition_count + 1
  ...
  {
    name="defines a minimum and maximum bitrate from FFmpeg", filter=hls.bandwidth, manifest=ffmpeg_master, context={min=1500000, max=1500000},
    check=function(modified_manifest)
      local rendition_count = 0
      for _ in string.gmatch(modified_manifest, "(BANDWIDTH=%d+)") do
        rendition_count = rendition_count + 1
  ...
  {
    name="returns all renditions when all renditions are filtered from FFmpeg", filter=hls.bandwidth, manifest=ffmpeg_master, context={min=1500, max=1500},
    check=function(modified_manifest)
      local rendition_count = 0
      for _ in string.gmatch(modified_manifest, "(BANDWIDTH=%d+)") do
        rendition_count = rendition_count + 1
  ...
  {
    name="returns all renditions when no context is passed from FFmpeg", filter=hls.bandwidth, manifest=ffmpeg_master, context={},
    check=function(modified_manifest)
      local rendition_count = 0
      for _ in string.gmatch(modified_manifest, "(BANDWIDTH=%d+)") do
        rendition_count = rendition_count + 1
